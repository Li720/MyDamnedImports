        ...
        do {
            ...
            for (klass, statement) in classDictionary {
                if newContents.contains("#import <\(oldFramework)/\(klass).h>") {
                    if newContents.contains("@import \(umbrellaFileName)") || newContents.contains("#import <\(umbrellaFileName)/\(umbrellaFileName).h>") {
                        newContents.replaceOccurrences(of: "#import <\(oldFramework)/\(klass).h>\n", with: "", options: NSString.CompareOptions(rawValue: 0) , range: NSMakeRange(0, newContents.length))
                    } else {
...
func helpMeWithImportsPlease(_ folder:String) {
    
    
    let umbrellaNameString = "TWRConnectedVehicle"
    let find = DemonFileFinder.init().findFilesRecursively(in:NSString(string:folder).expandingTildeInPath)
    print(find)
    let classes = AngelUmbrellaHeaderParser.init().findAvailableClassFiles(in:NSString(string:"~/workspace/random-project-folder/Externals/TowerPain/TWRConnectedVehicle/TWRConnectedVehicle/VHA/VHA.h").expandingTildeInPath)
    print(classes)
    for file in find.headerFiles {
        DemonFileParser.init().exorciseAndBlessImports(in:file, ofType: .header, using: classes,withUmbrellaFile:umbrellaNameString,replacing: "TowerPain")
    }
    
    for file in find.specFiles {
        DemonFileParser.init().exorciseAndBlessImports(in:file, ofType: .spec, using: classes,withUmbrellaFile:umbrellaNameString,replacing: "TowerPain")
    }
    
    for file in find.impFiles {
        DemonFileParser.init().exorciseAndBlessImports(in:file, ofType: .implementation, using: classes,withUmbrellaFile:umbrellaNameString,replacing: "TowerPain")
    }
}

helpMeWithImportsPlease("~/workspace/random-project-folder/randomproject-common")
helpMeWithImportsPlease("~/workspace/random-project-folder/randomproject-ap")
helpMeWithImportsPlease("~/workspace/random-project-folder/randomproject-eu")
helpMeWithImportsPlease("~/workspace/random-project-folder/randomproject-na")
helpMeWithImportsPlease("~/workspace/random-project-folder/randomprojectAPTests")
helpMeWithImportsPlease("~/workspace/random-project-folder/randomprojectEUTests")
helpMeWithImportsPlease("~/workspace/random-project-folder/randomprojectNATests")
helpMeWithImportsPlease("~/workspace/random-project-folder/randomprojectTests")

//helpMeWithImportsPlease("~/workspace/henry-clutch-ios/lincolnapp/lincolnapp-common")
//helpMeWithImportsPlease("~/workspace/henry-clutch-ios/lincolnapp/lincolnapp-na")
//helpMeWithImportsPlease("~/workspace/henry-clutch-ios/lincolnapp/lincolnappNaTests")
//helpMeWithImportsPlease("~/workspace/henry-clutch-ios/lincolnapp/lincolnappTests")
